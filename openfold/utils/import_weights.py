...
def generate_translation_dict(model, version, is_multimer=False, chain_mask=False, num_chains=None):
    ...
    if not(chain_mask):
        ...
        AttentionParams = lambda att: {
            **{f"query_w_{i}": LinearWeightMHA(att.linear_q_layers[i].weight) for i in range(0,num_chains)},           
            **{f"key_w_{i}": LinearWeightMHA(att.linear_k_layers[i].weight) for i in range(0,num_chains)},         
            **{f"value_w_{i}": LinearWeightMHA(att.linear_v_layers[i].weight) for i in range(0,num_chains)},            
            **{f"output_w_{i}": Param(att.linear_o_layers[i].weight, param_type=ParamType.LinearMHAOutputWeight) for i in range(0,num_chains)}, 
            **{f"output_b_{i}": LinearBias(att.linear_o_layers[i].bias) for i in range(0,num_chains)},
        }
        ...
        #for GlobalAttentionParams
        AttentionParamsOrig = lambda att: {
            "query_w": LinearWeightMHA(att.linear_q.weight),
            "key_w": LinearWeightMHA(att.linear_k.weight),
            "value_w": LinearWeightMHA(att.linear_v.weight),
            "output_w": Param(
        ...
        AttentionGatedParamsOrig = lambda att: dict(
            **AttentionParamsOrig(att),
            **{
                "gating_w": LinearWeightMHA(att.linear_g.weight),
                "gating_b": LinearBiasMHA(att.linear_g.bias),
            },
        ...
        #for TemplatePairStack
        TriAttParamsOrig = lambda tri_att: {
            "query_norm": LayerNormParams(tri_att.layer_norm),
            "feat_2d_weights": LinearWeight(tri_att.linear.weight),
            "attention": AttentionGatedParamsOrig(tri_att.mha),
        }
    ...
    def TriMulOutParams(tri_mul, outgoing=True):
        if re.fullmatch("^model_[1-5]_multimer_v3$", version):
            ...
            if not(chain_mask):
                ...
                d = {
                    "left_norm_input": LayerNormParams(tri_mul.layer_norm_in),
                    **{f"projection_{i}": lin_param_type(tri_mul.linear_ab_p_layers[i]) for i in range(0,num_chains)},
                    "gate": lin_param_type(tri_mul.linear_ab_g),
                    "center_norm": LayerNormParams(tri_mul.layer_norm_out),
                }
    ...
    #for TemplatePairStack
    def TriMulOutParamsOrig(tri_mul, outgoing=True):
        if re.fullmatch("^model_[1-5]_multimer_v3$", version):
            lin_param_type = LinearParams if outgoing else LinearParamsSwap
            d = {
                "left_norm_input": LayerNormParams(tri_mul.layer_norm_in),
    ...
    TriMulInParamsOrig = partial(TriMulOutParamsOrig, outgoing=False)
    ...
    if not(chain_mask):
        ...
        TemplatePairBlockParams = lambda b: {
            "triangle_attention_starting_node": TriAttParamsOrig(b.tri_att_start),
            "triangle_attention_ending_node": TriAttParamsOrig(b.tri_att_end),
            "triangle_multiplication_outgoing": TriMulOutParamsOrig(b.tri_mul_out),
            "triangle_multiplication_incoming": TriMulInParamsOrig(b.tri_mul_in),
            "pair_transition": PairTransitionParams(b.pair_transition),
        }
    ...
    if not(chain_mask):
        ...
        OuterProductMeanParams = lambda o: {
            "layer_norm_input": LayerNormParams(o.layer_norm),
            **{f"left_projection_{i}": LinearParams(o.linear_1_layers[i]) for i in range(0,num_chains)}, 
            **{f"right_projection_{i}": LinearParams(o.linear_2_layers[i]) for i in range(0,num_chains)},  
            "output_w": LinearWeightOPM(o.linear_out.weight),
            "output_b": LinearBias(o.linear_out.bias),
        }
    ...
    def EvoformerBlockParams(b, is_extra_msa=False):
        ...
        #if is_extra_msa:
        #    for attr in dir(b.pair_stack.tri_mul_out):
        #        print(f"{attr}: {getattr(b.pair_stack.tri_mul_out, attr)}")
...
def load_jax_weights_chainmask(npz_orig_path, npz_chainmask_path, num_chains):

    data = np.load(npz_orig_path)

    if os.path.exists(npz_chainmask_path):
        data_w_chainmask = np.load(npz_chainmask_path)
    else:
        data_w_chainmask = {key: data[key] for key in data.files}
        for block in ['extra_msa_stack', 'evoformer_iteration']:
            for msa_type in ['msa_row_attention_with_pair_bias','msa_column_attention']:
                for param in ['query_w','key_w','value_w','output_w','output_b']:
                    for i in range(0,num_chains):
                        chainmask_param = '%s_%d' % (param, i)
                        orig_param_key = 'alphafold/alphafold_iteration/evoformer/%s/%s/attention//%s' % (block,msa_type,param) 
                        chainmask_param_key = 'alphafold/alphafold_iteration/evoformer/%s/%s/attention//%s' % (block,msa_type,chainmask_param) 
                        print(chainmask_param_key)
                        if orig_param_key in data:
                            data_w_chainmask[chainmask_param_key] = np.copy(data[orig_param_key])
                        else:
                            print('%s does not exist in %s' % (orig_param_key, npz_orig_path))
        for block in ['extra_msa_stack', 'evoformer_iteration']:
            for tri_attn_type in ['triangle_attention_starting_node','triangle_attention_ending_node']:
                for param in ['query_w','key_w','value_w','output_w','output_b']:
                    for i in range(0,num_chains):
                        chainmask_param = '%s_%d' % (param, i)
                        orig_param_key = 'alphafold/alphafold_iteration/evoformer/%s/%s/attention//%s' % (block,tri_attn_type,param) 
                        chainmask_param_key = 'alphafold/alphafold_iteration/evoformer/%s/%s/attention//%s' % (block,tri_attn_type,chainmask_param) 
                        print(chainmask_param_key)
                        if orig_param_key in data:
                            data_w_chainmask[chainmask_param_key] = np.copy(data[orig_param_key])
                        else:
                            print('%s does not exist in %s' % (orig_param_key, npz_orig_path))
        for block in ['extra_msa_stack', 'evoformer_iteration']:
            for tri_mul_type in ['triangle_multiplication_incoming', 'triangle_multiplication_outgoing']:
                for param in ['weights', 'bias']:
                    for i in range(0,num_chains):
                        orig_param_key = 'alphafold/alphafold_iteration/evoformer/%s/%s/projection//%s' % (block,tri_mul_type,param) 
                        chainmask_param_key = 'alphafold/alphafold_iteration/evoformer/%s/%s/projection_%d//%s' % (block,tri_mul_type,i,param) 
                        print(chainmask_param_key)
                        if orig_param_key in data:
                            data_w_chainmask[chainmask_param_key] = np.copy(data[orig_param_key])
                        else:
                            print('%s does not exist in %s' % (orig_param_key, npz_orig_path))
        for block in ['extra_msa_stack', 'evoformer_iteration']:
            for proj_type in ['left_projection', 'right_projection']:
                for param in ['weights', 'bias']:
                    for i in range(0,num_chains):
                        orig_param_key = 'alphafold/alphafold_iteration/evoformer/%s/outer_product_mean/%s//%s' % (block,proj_type,param) 
                        chainmask_param_key = 'alphafold/alphafold_iteration/evoformer/%s/outer_product_mean/%s_%d//%s' % (block,proj_type,i,param) 
                        print(chainmask_param_key)
                        if orig_param_key in data:
                            data_w_chainmask[chainmask_param_key] = np.copy(data[orig_param_key])
                        else:
        ...
        print('saving %s' % npz_chainmask_path)
        np.savez(npz_chainmask_path, **data_w_chainmask)
...
def import_jax_weights_(config, model, npz_path, version="model_1"):
    ...
    chain_mask = False 
    num_chains = None
    if 'custom_fine_tuning' in config:
    ...
    # Sanity check
    keys = list(data.keys()) #jax keys
    flat_keys = list(flat.keys()) #pytorch keys
    incorrect = [k for k in flat_keys if k not in keys]
    missing = [k for k in keys if k not in flat_keys]

    print(f"Not in JAX but in PYTORCH: {incorrect}")
    print(f"Not in PYTORCH but in JAX: {missing}")
