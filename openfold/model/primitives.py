...
class AttentionChainMask(nn.Module):
    """
    Standard multi-head attention using AlphaFold's default layer
    initialization. Allows multiple bias vectors.
    """
    def __init__(
        ...
        Args:
            ...
            gating:
                Whether the output should be gated using query data
        """
        super(AttentionChainMask, self).__init__()
        ...
        if torch.is_tensor(self.chain_mask):
            ...
            self.linear_q_layers = nn.ModuleList([Linear(
                self.c_q, self.c_hidden * self.no_heads, bias=False, init="glorot"
            ) for _ in range(num_chains)])
            self.linear_k_layers = nn.ModuleList([Linear(
                self.c_k, self.c_hidden * self.no_heads, bias=False, init="glorot"
            ) for _ in range(num_chains)])
            self.linear_v_layers = nn.ModuleList([Linear(
                self.c_v, self.c_hidden * self.no_heads, bias=False, init="glorot"
            ) for _ in range(num_chains)])
            self.linear_o_layers = nn.ModuleList([Linear(
                self.c_hidden * self.no_heads, self.c_q, init="final"
            ) for _ in range(num_chains)])
        else:
            # DISCREPANCY: c_hidden is not the per-head channel dimension, as
    ...
    def _prep_qkv(self,
        ...
        '''layer_0 = self.linear_q_layers[0](q_x)
        mask_0 = self.chain_mask[0]
        layer_1 = self.linear_q_layers[1](q_x)
        mask_1 = self.chain_mask[1]

        lm_0 = layer_0*mask_0
        lm_1 = layer_1*mask_1

        print('mask')
        print(self.chain_mask)
        print('stack')
        print(torch.stack([lm_0,lm_1]))
        print('sum')
        print(torch.sum(torch.stack([lm_0,lm_1])))''' 

        if torch.is_tensor(self.chain_mask):
            q = torch.sum(torch.stack([layer(q_x)*cmask for cmask,layer in zip(self.chain_mask,self.linear_q_layers)]),dim=0)
            k = torch.sum(torch.stack([layer(kv_x)*cmask for cmask,layer in zip(self.chain_mask,self.linear_k_layers)]),dim=0)
            v = torch.sum(torch.stack([layer(kv_x)*cmask for cmask,layer in zip(self.chain_mask,self.linear_v_layers)]),dim=0)
        else:
            q = self.linear_q(q_x)
            k = self.linear_k(kv_x)
            v = self.linear_v(kv_x)
    ...
    def _wrap_up(self,
        ...
        # [*, Q, C_q]
        if torch.is_tensor(self.chain_mask):
            o = torch.sum(torch.stack([layer(o)*cmask for cmask,layer in zip(self.chain_mask,self.linear_o_layers)]),dim=0)
        else:
            o = self.linear_o(o)
