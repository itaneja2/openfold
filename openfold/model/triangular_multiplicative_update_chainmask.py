...
class FusedTriangleMultiplicativeUpdateChainMask(BaseTriangleMultiplicativeUpdate):
    """
    Implements Algorithms 11 and 12.
    """

    def __init__(self, c_z, c_hidden, chain_mask=None, _outgoing=True):
        """
        Args:
            c_z:
                Input channel dimension
            c:
                Hidden channel dimension
        """
        super(FusedTriangleMultiplicativeUpdateChainMask, self).__init__(c_z=c_z,
                                                                c_hidden=c_hidden,
                                                                _outgoing=_outgoing)

        self.chain_mask = chain_mask

        if torch.is_tensor(self.chain_mask):
            num_chains = chain_mask.shape[0]
            self.linear_ab_p_layers = nn.ModuleList([Linear(c_z, c_hidden * 2) for _ in range(num_chains)])
        else:
    ...
    def _inference_forward(self,
        ...
        def compute_projection_helper(pair, mask):
            ...
            if torch.is_tensor(self.chain_mask):
                p *= torch.sum(torch.stack([layer(pair)*cmask for cmask,layer in zip(self.chain_mask,self.linear_ab_p_layers)]),dim=0)
            else:
    ...
    def forward(self,
        ...
        if torch.is_tensor(self.chain_mask):
            ab = ab * torch.sum(torch.stack([layer(pair)*cmask for cmask,layer in zip(self.chain_mask,self.linear_ab_p_layers)]),dim=0)
        else:
...
class FusedTriangleMultiplicationOutgoingChainMask(FusedTriangleMultiplicativeUpdateChainMask):
    """
    Implements Algorithm 11.
    """
    __init__ = partialmethod(FusedTriangleMultiplicativeUpdateChainMask.__init__, _outgoing=True)


class FusedTriangleMultiplicationIncomingChainMask(FusedTriangleMultiplicativeUpdateChainMask):
    """
    Implements Algorithm 12.
    """
    __init__ = partialmethod(FusedTriangleMultiplicativeUpdateChainMask.__init__, _outgoing=False)
